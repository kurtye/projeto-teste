...
import { clans, getClanFromPlayerName } from './src/lib/clans';
...
async function main() {
  try {
    ...
    fs.writeFileSync('players_output.json', JSON.stringify(result, null, 2));
    console.log('Finished writing players_output.json');
    process.exit(0);
  } catch (err) {
    console.error(err);
    process.exit(1);
